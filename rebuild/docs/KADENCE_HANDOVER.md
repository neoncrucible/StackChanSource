...
**Current candidate:** RC2 speech recovery and Signal Console, host 0.3.2 / unchanged firmware 0.21.2. Boss confirms the 0.21.2 robot stays booted and touch is much more responsive. This host update does not require reflashing that firmware. RC2 voice remains awaiting physical confirmation. The implemented approved scope, ownership details, automated checks and physical acceptance procedure are in [RC2_ENGINEERING.md](RC2_ENGINEERING.md). Match source with the package `RELEASE.json`. New hardware features are not yet owner-signed-off.
...
**Current voice finding and next physical check:** The owner's latest diagnostic reaches TTS (one turn after reasoning), then touch-cancels. It does not identify the failing TTS substage. Cross-reference with approved RC1 proves the original Sonia adapter/decoder were unchanged; Windows source and frozen synthetic tests both render successfully. Host 0.3.2 preserves Sonia, bounds its full process lifetime, and provides an explicit local Windows voice fallback. Do not present the PC-specific cause as established. Deliver the tested host update, without flashing 0.21.2, and request one audible turn, cancellation and a second turn. Detailed evidence and deadlines are in RC2_ENGINEERING.md. The new diagnostic substages identify connection/audio/decode/fallback.
