...
constexpr int kOutputOpenAttempts = 3;
...
bool initialise_input_stage(InputStage* stage)
{
    ...
    if (i2s_new_channel(&chan_cfg, nullptr, &stage->rx) != ESP_OK) return false;
    ...
    if (i2s_channel_init_tdm_mode(stage->rx, &tdm_cfg) != ESP_OK) return false;
    if (i2s_channel_enable(stage->rx) != ESP_OK) return false;

    audio_codec_i2s_cfg_t data_cfg{};
    data_cfg.port = I2S_NUM_0;
    data_cfg.rx_handle = stage->rx;
    data_cfg.tx_handle = nullptr;
    stage->data_if = audio_codec_new_i2s_data(&data_cfg);
    if (stage->data_if == nullptr) return false;
    ...
    stage->ctrl_if = audio_codec_new_i2c_ctrl(&ctrl_cfg);
    if (stage->ctrl_if == nullptr) return false;

    es7210_codec_cfg_t codec_cfg{};
    codec_cfg.ctrl_if = stage->ctrl_if;
    ...
    stage->codec_if = es7210_codec_new(&codec_cfg);
    if (stage->codec_if == nullptr) return false;

    esp_codec_dev_cfg_t dev_cfg{};
    dev_cfg.dev_type = ESP_CODEC_DEV_TYPE_IN;
    dev_cfg.codec_if = stage->codec_if;
    dev_cfg.data_if = stage->data_if;
    stage->dev = esp_codec_dev_new(&dev_cfg);
    if (stage->dev == nullptr) return false;
    ...
    if (esp_codec_dev_open(stage->dev, &sample_info) != ESP_OK) return false;
...
bool sample_input(InputStage* stage, const char* label)
{
    if (stage == nullptr || !stage->ready || stage->dev == nullptr) return false;
...
bool stop_input_stage(InputStage* stage)
{
    if (stage == nullptr) return false;
    bool ok = true;
    if (stage->dev != nullptr) {
        const esp_err_t close_err = static_cast<esp_err_t>(esp_codec_dev_close(stage->dev));
        if (close_err != ESP_OK) ok = false;
    }
    if (stage->rx != nullptr) {
        const esp_err_t disable_err = i2s_channel_disable(stage->rx);
        if (disable_err != ESP_OK && disable_err != ESP_ERR_INVALID_STATE) ok = false;
        if (i2s_del_channel(stage->rx) != ESP_OK) ok = false;
        stage->rx = nullptr;
    }
    stage->ready = false;
    return ok;
...
bool initialise_output_stage(OutputStage* stage)
{
    if (stage == nullptr) return false;
    ...
    chan_cfg.dma_desc_num = 6;
    chan_cfg.dma_frame_num = 240;
    if (i2s_new_channel(&chan_cfg, &stage->tx, nullptr) != ESP_OK) return false;
    ...
    if (i2s_channel_init_std_mode(stage->tx, &std_cfg) != ESP_OK) return false;
    if (i2s_channel_enable(stage->tx) != ESP_OK) return false;

    audio_codec_i2s_cfg_t data_cfg{};
    data_cfg.port = I2S_NUM_0;
    data_cfg.rx_handle = nullptr;
    data_cfg.tx_handle = stage->tx;
    stage->data_if = audio_codec_new_i2s_data(&data_cfg);
    if (stage->data_if == nullptr) return false;
    ...
    stage->gpio_if = audio_codec_new_gpio();
    if (stage->gpio_if == nullptr) return false;

    aw88298_codec_cfg_t codec_cfg{};
    codec_cfg.ctrl_if = stage->ctrl_if;
    codec_cfg.gpio_if = stage->gpio_if;
    ...
    stage->codec_if = aw88298_codec_new(&codec_cfg);
    if (stage->codec_if == nullptr) return false;
    ...
    for (int attempt = 1; attempt <= kOutputOpenAttempts; ++attempt) {
        open_err = static_cast<esp_err_t>(esp_codec_dev_open(stage->dev, &sample_info));
        ...
        if (attempt < kOutputOpenAttempts) vTaskDelay(pdMS_TO_TICKS(25));
    }
    if (open_err != ESP_OK) return false;

    const esp_err_t volume_err = static_cast<esp_err_t>(esp_codec_dev_set_out_vol(stage->dev, kOutputVolume));
    if (volume_err != ESP_OK) return false;
...
bool run_output_stage(OutputStage* stage)
{
    if (stage == nullptr || !stage->ready || stage->dev == nullptr) return false;
    build_probe_signal();
...
bool stop_output_stage(OutputStage* stage)
{
    if (stage == nullptr) return false;
    bool ok = true;
    if (stage->dev != nullptr) {
        const esp_err_t mute_err = static_cast<esp_err_t>(esp_codec_dev_set_out_mute(stage->dev, true));
        if (mute_err != ESP_OK) ok = false;
        const esp_err_t close_err = static_cast<esp_err_t>(esp_codec_dev_close(stage->dev));
        if (close_err != ESP_OK) ok = false;
    }
    if (stage->tx != nullptr) {
        const esp_err_t disable_err = i2s_channel_disable(stage->tx);
        if (disable_err != ESP_OK && disable_err != ESP_ERR_INVALID_STATE) ok = false;
        if (i2s_del_channel(stage->tx) != ESP_OK) ok = false;
        stage->tx = nullptr;
    }
    stage->ready = false;
    return ok;
...
bool run_probe7()
{
    ...
    InputStage first_input{};
    if (!initialise_input_stage(&first_input)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-a-init");
        return false;
    }
    if (!sample_input(&first_input, "stage-a")) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-a-sample");
        return false;
    }
    if (!stop_input_stage(&first_input)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-a-stop");
        return false;
    }
    ...
    OutputStage output{};
    if (!initialise_output_stage(&output)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-b-init");
        return false;
    }
    if (!run_output_stage(&output)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-b-run");
        return false;
    }
    if (!stop_output_stage(&output)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-b-stop");
        return false;
    }
    ...
    InputStage second_input{};
    if (!initialise_input_stage(&second_input)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-c-init");
        return false;
    }
    if (!sample_input(&second_input, "stage-c")) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-c-sample");
        return false;
    }
    if (!stop_input_stage(&second_input)) {
        ESP_LOGE(kLogTag, "PROBE7 status=failed stage=stage-c-stop");
        return false;
    }

    ESP_LOGI(kLogTag, "PROBE7 status=complete sequence=3");
    return true;
}
