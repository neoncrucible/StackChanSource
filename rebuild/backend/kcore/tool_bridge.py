...
class KadenceToolBoundary:
    ...
    def register(self, spec: KadenceToolSpec) -> None:
        ...
        if not inspect.iscoroutinefunction(spec.handler):
            raise ToolRegistrationError("handlers must be cooperative async functions")
        if not math.isfinite(spec.timeout) or not 0 < spec.timeout <= 30:
            raise ToolRegistrationError("tool timeout must be in (0, 30]")
        ...
        self._tools[spec.name] = KadenceToolSpec(
            spec.name, spec.description, copy.deepcopy(spec.parameters),
            spec.handler, spec.timeout, spec.writes,
        )
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "requires_confirmation": spec.writes,
                },
            }
    ...
    def validate(self, name: Any, arguments: Any) -> Optional[Dict[str, Any]]:
        if not isinstance(name, str) or name not in self._tools:
            return self._reject(
                name[:64] if isinstance(name, str) else None,
                "unknown_tool",
        ...
        try:
            ...
            if len(json.dumps(arguments, allow_nan=False)) > 8192:
                raise ToolValidationError("arguments exceed limit")
            self._validate_value(self._tools[name].parameters, arguments, path="$arguments")
        except (ToolValidationError, TypeError, ValueError, RecursionError):
            return self._reject(name, "invalid_arguments", "Arguments did not match the registered schema.")
        return None
    ...
    async def invoke(self, name: str, arguments: dict) -> dict:
        """ToolBridge entry point: provider proposals cannot authorise writes."""
        return await self.execute(name, arguments)

    async def execute(self, name: Any, arguments: Any, *, confirmed: bool = False) -> dict:
        rejection = self.validate(name, arguments)
        if rejection is not None:
            return rejection
        spec = self._tools[name]
        if self._closed or len(self._running) >= 4:
            return self._reject(name, "unavailable", "Tool service is unavailable.")
        if spec.writes and confirmed is not True:
            return self._reject(name, "denied", "This change needs your confirmation.")

        # Only project-owned async handlers are registered. Blocking I/O belongs
        # in a bounded worker, never on the event loop. A handler swallowing
        # cancellation is quarantined and cannot hold voice/serial ownership.
        task = asyncio.create_task(spec.handler(copy.deepcopy(arguments)))
        ...
        try:
            ...
            if not done:
                task.cancel()
                return self._reject(name, "timeout", "Tool timed out; its outcome is not confirmed.")
            result = task.result()
        except asyncio.CancelledError:
            task.cancel()
            raise
        except Exception:
            return self._reject(name, "execution_error", "Tool execution failed safely.")
    ...
    def _finished(self, task: asyncio.Task) -> None:
        ...
        if not task.cancelled():
            task.exception()  # Retrieve exceptions from quarantined late tasks.
