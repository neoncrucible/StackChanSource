    ...
    def test_real_worker_timezone_project_refresh_privacy_and_shutdown(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                window.on_event('audio_link_test',{'state':'testing'})
                window.on_event('voice_progress',{'stage':'connecting','timeout_ms':30000})
                self.assertIn('microphone stays off',window.voice_health.text())
                self.assertNotIn('recording cue',window.voice_health.text())
                window.on_event('audio_link_test',{'state':'failed'})
                self.assertIn('failed',window.voice_health.text())
                window.on_event('voice_progress',{'stage':'connecting','timeout_ms':30000})
                self.assertIn('wait for the recording cue',window.voice_health.text())
                with patch.object(window.control,'send') as send:
