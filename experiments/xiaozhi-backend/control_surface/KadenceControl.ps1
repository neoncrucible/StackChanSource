...
if (-not (Test-Path $StartScript)) {
    [System.Windows.Forms.MessageBox]::Show(
        "Alpha 2 launcher not found:`r`n$StartScript",
...
$script:ProcessExitReported = $false
$script:ServerReady = $false
$script:RobotConnected = $false
$script:AsrReady = $false
...
$clearButton.FlatAppearance.BorderColor = $ColorOffline
$clearButton.BackColor = $ColorPanelAlt
$clearButton.ForeColor = $ColorMuted
$clearButton.Font = New-Font 8
...
function Get-EyeColor {
    switch ($script:EyeState) {
        "booting" { return $ColorAmber }
        "online" { return $ColorCyan }
        "connected" { return $ColorTeal }
...
function Set-OfflineState {
    ...
    $script:RobotConnected = $false
    $script:AsrReady = $false
    $serverHealth.Text = "SERVER  OFFLINE"
    $serverHealth.ForeColor = $ColorOffline
    $robotHealth.Text = "ROBOT  DISCONNECTED"
    $robotHealth.ForeColor = $ColorOffline
    $asrHealth.Text = "ASR  WAITING"
    $asrHealth.ForeColor = $ColorOffline
    $stateLabel.Text = "SYSTEM OFFLINE"
    $stateLabel.ForeColor = $ColorOffline
...
function Process-ServerLine {
    ...
    if ($Line -match '=== Kadence 2\.0 Alpha 2 ===') {
        $stateLabel.Text = "INITIALISING"
        $stateLabel.ForeColor = $ColorAmber
        $serverHealth.Text = "SERVER  BOOTING"
        $serverHealth.ForeColor = $ColorAmber
        Set-EyeState "booting"
    }

    if ($Line -match 'Injected canonical Kadence persona') {
        $personaHealth.Text = "PERSONA  CANONICAL V1"
        $personaHealth.ForeColor = $ColorCyan
    }

    if (($Line -match 'Websocket') -and ($Line -match 'ws://')) {
        $script:ServerReady = $true
        $serverHealth.Text = "SERVER  ONLINE"
        $serverHealth.ForeColor = $ColorCyan
        if (-not $script:RobotConnected) {
            $stateLabel.Text = "SERVER ONLINE"
            $stateLabel.ForeColor = $ColorCyan
            Set-EyeState "online"
        }
    }

    if ($Line -match 'K2 ASR LIVE ready:') {
        $script:AsrReady = $true
        $asrHealth.Text = "ASR  READY"
        $asrHealth.ForeColor = $ColorTeal
    }

    if ($Line -match ' conn - Headers:') {
        $script:RobotConnected = $true
        $robotHealth.Text = "ROBOT  CONNECTED"
        $robotHealth.ForeColor = $ColorTeal
        $stateLabel.Text = "KADENCE ONLINE"
        $stateLabel.ForeColor = $ColorTeal
    ...
    if ($Line -match '初始化组件: llm成功\s+(.+)$') {
        $modelHealth.Text = "MODEL  " + $Matches[1].Trim().ToUpperInvariant()
    }

    if (($Line -match 'ERROR') -or ($Line -match 'Traceback') -or ($Line -match 'exception')) {
        if (-not $script:StopRequested) {
            $stateLabel.Text = "ATTENTION REQUIRED"
            $stateLabel.ForeColor = $ColorRed
            Set-EyeState "error"
        }
    }
}
...
function Start-Backend {
    ...
    $startButton.ForeColor = $ColorOffline
    $stopButton.Enabled = $true
    $stopButton.ForeColor = $ColorRed
    $stopButton.FlatAppearance.BorderColor = $ColorRed
    ...
    $process.add_OutputDataReceived({
        param($sender, $e)
        if ($null -ne $e.Data) {
            $script:LogQueue.Enqueue($e.Data)
        }
    })
    ...
    $process.add_ErrorDataReceived({
        param($sender, $e)
        if ($null -ne $e.Data) {
            $script:LogQueue.Enqueue("[stderr] " + $e.Data)
        }
    })

    try {
        if (-not $process.Start()) {
            throw "Windows failed to start the Alpha 2 launcher."
        }
        $script:BackendProcess = $process
        $process.BeginOutputReadLine()
        $process.BeginErrorReadLine()
        Append-Log ("[KADENCE UI] Backend process PID {0}" -f $process.Id)
    }
    catch {
        Append-Log ("[KADENCE UI] START FAILED: " + $_.Exception.Message)
        $serverHealth.Text = "SERVER  FAILED"
        $serverHealth.ForeColor = $ColorRed
        $stateLabel.Text = "START FAILED"
        $stateLabel.ForeColor = $ColorRed
        Set-EyeState "error"
...
function Stop-Backend {
    ...
    $stateLabel.Text = "SHUTTING DOWN"
    $stateLabel.ForeColor = $ColorAmber
    $serverHealth.Text = "SERVER  STOPPING"
    $serverHealth.ForeColor = $ColorAmber
    Set-EyeState "booting"

    if (-not $Silent) {
        Append-Log "[KADENCE UI] Stopping backend process tree..."
    }
    ...
    try {
        ...
        if (($taskkill.ExitCode -ne 0) -and (-not $script:BackendProcess.HasExited)) {
            throw "taskkill exited with code $($taskkill.ExitCode)."
        }
    }
    catch {
        ...
        try {
            ...
            Append-Log ("[KADENCE UI] STOP FAILED: " + $_.Exception.Message)
        }
    }
}
...
$timer.add_Tick({
    ...
    while ($script:LogQueue.TryDequeue([ref]$line)) {
        Process-ServerLine $line
        $line = $null
    }

    if (($null -ne $script:BackendProcess) -and $script:BackendProcess.HasExited -and (-not $script:ProcessExitReported)) {
        $script:ProcessExitReported = $true
        ...
        if ($script:StopRequested) {
            ...
            $serverHealth.Text = "SERVER  EXITED"
            $serverHealth.ForeColor = $ColorRed
            $robotHealth.Text = "ROBOT  DISCONNECTED"
            $robotHealth.ForeColor = $ColorOffline
            $asrHealth.Text = "ASR  WAITING"
            $asrHealth.ForeColor = $ColorOffline
            $stateLabel.Text = "BACKEND EXITED"
            $stateLabel.ForeColor = $ColorRed
            Set-EyeState "error"
        }
...
$form.add_FormClosing({
    ...
    if (($null -ne $script:BackendProcess) -and (-not $script:BackendProcess.HasExited)) {
        $answer = [System.Windows.Forms.MessageBox]::Show(
            "The Kadence backend is still running. Stop it and exit?",
            "Kadence Control Surface",
...
Append-Log "[KADENCE UI] Provider switching remains deferred until the benchmark milestone."
