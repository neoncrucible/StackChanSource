...
foreach ($Path in @($UiScript,$PatchV4,$PatchV41,$PatchV43,$PatchV44,$PatchV45,$ApplyM7,$Launcher)) {
    if (-not (Test-Path $Path)) {
        throw "Missing M7 Control Surface test dependency: $Path"
    }
}

# Render only the last physically launchable UI chain. V4.6 is intentionally
# quarantined after a physical exit-code-1 regression and is not part of the
# stable launcher until separately repaired and physically accepted.
...
Expect-Marker '[KADENCE UI] Control Surface V4.5 ready.' 'stable V4.5 render identified'

$LauncherText = [System.IO.File]::ReadAllText($Launcher,[System.Text.Encoding]::UTF8)
if ($LauncherText.Contains('$PatchScriptV46') -or $LauncherText.Contains('& $PatchScriptV46')) {
    throw 'FAIL  unstable V4.6 patch is still in the stable launch chain'
}
if (-not $LauncherText.Contains('V4.6 is intentionally excluded')) {
    throw 'FAIL  stable launcher does not record the V4.6 quarantine'
}
Write-Host 'PASS  V4.6 is quarantined from the stable launch chain'
...
Write-Host 'M7 Control Surface stable-chain test: PASS'
